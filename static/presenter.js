...
(function () {
  ...
  function updateRecentSubmissions(submissions) {
    ...
    submissions
      ...
      .forEach((submission) => {
        ...
        usernameSpan.textContent = showCrustacean
          ? "Crustacean"
          : submission.username;
      ...
      fetch(url)
        ...
        .then((data) => {
          ...
          if (all_submissions.length > 0) {
            ...
            lastTimestamp =
              all_submissions[all_submissions.length - 1].timestamp;
          }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document
      .getElementById("toggle-answer")
      .addEventListener("click", function () {
        blurAnswer = !blurAnswer;
        document
          .getElementById("answer")
          .classList.toggle("blurred-text", blurAnswer);
    ...
    document
      ...
      .addEventListener("click", function () {
        isBlurred = !isBlurred;
        document
          .querySelectorAll("#recent-submissions span:not(.username)")
          .forEach((span) => span.classList.toggle("blurred-text", isBlurred));
        this.textContent = isBlurred
          ? "Show Submission Text"
          : "Hide Submission Text";
      });
